#!/usr/bin/env python3
"""
Training script for PlonkModule.
...
from models.plonk import PlonkModule
...
def create_model(config: Dict[str, Any]) -> PlonkModule:
    """
    ...
    Returns:
        Initialized PlonkModule
    """
    model_config = config['model']
    model = PlonkModule(**model_config)
    return model
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Train PlonkModule')
    parser.add_argument(
        '--config', 
        type=str, 
        required=True,
    ...
    if len(train_loader) == 0:
        raise ValueError("Training dataloader is empty! Check your dataset configuration. "
                        f"Dataset has {len(train_loader.dataset)} samples but batch_size is {config['model']['batch_size']}. "
